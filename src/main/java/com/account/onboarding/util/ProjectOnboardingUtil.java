...
public class ProjectOnboardingUtil {
	...
	public int getRoleIdOfProjectOwner() {
		log.info("ProjectOnboardingUtil, method for getting role id of project owner");

		if (roleIdOfProjectOwner == 0) {
			log.info("Role id for project owner is finding for the first time");

			Criteria criteria = new Criteria().andOperator(
					Criteria.where(ProjectOnboardingConstant.TYPE_NAME).is(ProjectOnboardingConstant.ROLE),
					Criteria.where(ProjectOnboardingConstant.TYPE_DESC).is(ProjectOnboardingConstant.PROJECT_OWNER));
			Query query = new Query();
			query.addCriteria(criteria);
			query.fields().include(ProjectOnboardingConstant.TYPE_ID);
