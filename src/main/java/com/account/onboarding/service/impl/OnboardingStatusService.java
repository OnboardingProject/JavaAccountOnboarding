...
public class OnboardingStatusService {
	...
	public List<TaskDetailsReportResponse> createTaskDetailsReport(Project project, UserEntity user) //throws Exception
	{
		...
		List<TaskDetails> userTaskDetails = user.getProjectTasks().stream()
	 ...
	 */
	public TaskPercentageReportResponse createTaskPercentageReport(Project project, UserEntity user) {
		...
		List<ProjectTaskDetails> projectTaskDetails = user.getProjectTasks().stream()
				.filter(projectDetails -> projectDetails.getProjectId().equals(project.getProjectId()))
