...
public class ProjectTasksService {
	...
	public List<Task> getProjectTasksByProjectId(String projectId)// throws Exception 
	{
		...
		Query query = projectOnboardingUtil
				.createQuery(Criteria.where(ProjectOnboardingConstant.PROJECT_ID).is(projectId));
	...
	public Project addOrEditTask(ProjectTaskRequest projectTaskRequest) //throws Exception 
	{
		...
		Query query = projectOnboardingUtil.createQuery(
				Criteria.where(ProjectOnboardingConstant.PROJECT_ID).is(projectTaskRequest.getProjectId()));
		List<Project> project = mongoTemplate.find(query, Project.class);
		...
		if (!CollectionUtils.isEmpty(project)) {
			...
			if (project.get(0).getTasks() != null) {

				tasks = project.get(0).getTasks();
			}
			...
			update.set(ProjectOnboardingConstant.TASKS, tasks);
			 ...
			 * task/edited task
			 */
			query = projectOnboardingUtil.createQuery(Criteria.where(ProjectOnboardingConstant.DESIGNATION)
					.in(projectTaskRequest.getTask().getDesignation())
					.andOperator(Criteria.where(ProjectOnboardingConstant.PROJECT_TASKS_PROJ_ID)
							.in(projectTaskRequest.getProjectId())));
			...
			for (UserEntity user : users) {
				Query queryUser = projectOnboardingUtil.createQuery(Criteria
						.where(ProjectOnboardingConstant.DESIGNATION).in(projectTaskRequest.getTask().getDesignation())
						.and(ProjectOnboardingConstant.USER_ID).is(user.getUserId())
						.andOperator(Criteria.where(ProjectOnboardingConstant.PROJECT_TASKS_PROJ_ID)
								.in(projectTaskRequest.getProjectId())));

				for (ProjectTaskDetails projectTask : user.getProjectTasks()) {
					if (projectTask.getProjectId().equals(projectTaskRequest.getProjectId())) {

						if (projectTask.getTasks() != null) {
				...
				userUpdate.set(ProjectOnboardingConstant.PROJECT_TASKS, user.getProjectTasks());
				mongoTemplate.upsert(queryUser, userUpdate, UserEntity.class);
	...
	public List<Task> deleteTask(DeleteTaskRequest deleteTaskRequest)// throws Exception 
	{
		...
		query.addCriteria(Criteria.where(ProjectOnboardingConstant.PROJECT_ID).is(deleteTaskRequest.getProjectId()));
		List<Project> project = mongoTemplate.find(query, Project.class);
		...
		if (!CollectionUtils.isEmpty(project)) {
			...
			userDeleteQuery.addCriteria(Criteria.where(ProjectOnboardingConstant.PROJECT_TASKS_PROJ_ID)
					.is(deleteTaskRequest.getProjectId())
					.andOperator(Criteria.where(ProjectOnboardingConstant.PROJECTTASKS_TASK_ID).in(taskIdList)));

			userList = mongoTemplate.find(userDeleteQuery, UserEntity.class);

			for (UserEntity user : userList) {
				List<ProjectTaskDetails> projectTaskList = user.getProjectTasks().stream()
				...
				if (!CollectionUtils.isEmpty(tasksForUser)) {
					...
					Query userUpdateQuery = new Query(Criteria.where(ProjectOnboardingConstant.PROJECT_TASKS_PROJ_ID)
							.is(deleteTaskRequest.getProjectId())
							.andOperator(Criteria.where(ProjectOnboardingConstant.USER_ID).is(user.getUserId())));

					Update userUpdate = new Update();
					userUpdate.set(ProjectOnboardingConstant.PROJECTTASKS_TASKS, tasksForUser);
					mongoTemplate.upsert(userUpdateQuery, userUpdate, UserEntity.class);
				}
			}
			...
			Update update = new Update();
			update.set(ProjectOnboardingConstant.TASKS, projectTask);
			mongoTemplate.upsert(query, update, Project.class);
