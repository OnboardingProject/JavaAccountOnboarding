...
public class UserControllerTest {
	...
	public void addUserTest() throws JsonProcessingException, Exception{
		UserEntity user = new UserEntity("U2345", "Kiara",null, "abc", "Kiara", "Kevin", "kiara@abc.com", "9898988787", "java",
				new Date(), "U1234", "U1234", new Date(), 1,"", null, "U2345, U1234", null,null);
		...
		mockMvc.perform(MockMvcRequestBuilders.post("http://localhost:8080/user/add").contentType(MediaType.APPLICATION_JSON)
	...
	@Test
	    public void updateTest() throws Exception {
		 UserEntity user = new UserEntity("U2345", "Kiara",null, "abc", "Kiara", "Kevin", "kiara@abc.com", "9898988787", "java",
					new Date(), "U2345", "U2345", new Date(), 1,"", null, "U2345", null,null);
			UserEditRequest userEdit = new UserEditRequest("U2345", 1, "U2345");
	        when(userService.updateUser(any())).thenReturn(user);
	        mockMvc.perform(put("/user/update").contentType(MediaType.APPLICATION_JSON).content(asJsonString(userEdit)))
		  ...
		  @Test
			public void viewAllUserByHeirarchyTestSuccess() throws Exception {
				String userId = "u101";
				List<UserListResponse> userResponses = new ArrayList<>();
				...
				mockMvc.perform(MockMvcRequestBuilders.get("/user/views/u101").contentType(MediaType.APPLICATION_JSON))
						.andExpect(MockMvcResultMatchers.status().isOk());

			}
		  @Test
			public void viewByUserIdTestSuccess() throws Exception {
				String userId = "u101";
				UserResponse userResponse = new UserResponse();
				
				userResponse.setUserId("u101");
				userResponse.setUserName("Mayank Tripati");
				userResponse.setRoleId(101);
				
				when(userService.viewUser(userId)).thenReturn(userResponse);
				mockMvc.perform(MockMvcRequestBuilders.get("/user/u101").contentType(MediaType.APPLICATION_JSON))
						.andExpect(MockMvcResultMatchers.status().isOk());
			...
			public void viewAllUserByHeirarchyTestFailure() throws Exception {
				...
				mockMvc.perform(MockMvcRequestBuilders.get("/user/views/u1766").contentType(MediaType.APPLICATION_JSON))
			...
			public void findByFirstNameTestSuccess() throws JsonProcessingException, Exception {
				...
				mockMvc.perform(MockMvcRequestBuilders.get("/user/search/Mayank")
			...
			public void findByFirstNameTestFailure() throws JsonProcessingException, Exception {
				...
				userResponses.add(response);
				when(userService.findByFirstName(firstName)).thenReturn(userResponses);
				mockMvc.perform(MockMvcRequestBuilders.get("/user/search/amith").contentType(MediaType.APPLICATION_JSON)
